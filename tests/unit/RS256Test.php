...
class RS256Test extends Base
{
    /**
     * @test
     * @throws \Exception
     */
    public function shouldAbleToUseAlgoHS256() {
        $algo = new RS256();
         ...
         */
        $signature = $algo->sign($key, $input);
        $encodedSignature = Base64Url::encode($signature);

        /**
         * use public key to verify the input
         */
        $this->assertTrue($algo->verify($key, $input, $signature));
    }
}